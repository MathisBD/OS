...
mkdir -p $IMGDIR/progs
cp -r $SYSROOT/progs/. $IMGDIR/progs/.

# zeroed out disk
dd if=/dev/zero of=./$IMAGE bs=$DISK_BLOCK_SIZE count=$DISK_BLOCK_COUNT

LOOP_DEV=$(sudo losetup -Pf --show ./$IMAGE)
# format disk (create the filesystem) : EXT2 revision 0
sudo mkfs.ext2 $LOOP_DEV -b $DISK_BLOCK_SIZE -d $IMGDIR -v -r 0
